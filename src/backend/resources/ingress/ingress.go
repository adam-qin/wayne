...
func CreateOrUpdateIngress(c *kubernetes.Clientset, ingress *networkingv1.Ingress) (*Ingress, error) {
	old, err := c.NetworkingV1().Ingresses(ingress.Namespace).Get(ingress.Name, metaV1.GetOptions{})
	if err != nil {
		if errors.IsNotFound(err) {
			kubeIngress, err := c.NetworkingV1().Ingresses(ingress.Namespace).Create(ingress)
			if err != nil {
				return nil, err
			}
	...
	kubeIngress, err := c.NetworkingV1().Ingresses(ingress.Namespace).Update(old)
	if err != nil {
		return nil, err
	}
	return toIngress(kubeIngress), nil
...
func GetIngressDetail(c *kubernetes.Clientset, name, namespace string) (*Ingress, error) {
	ingress, err := c.NetworkingV1().Ingresses(namespace).Get(name, metaV1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
func DeleteIngress(c *kubernetes.Clientset, name, namespace string) error {
	return c.NetworkingV1().Ingresses(namespace).Delete(name, &metaV1.DeleteOptions{})
}

func GetIngressList(cli *kubernetes.Clientset, namespace string, opts metaV1.ListOptions) (list []*Ingress, err error) {
	kubeIngressList, err := cli.NetworkingV1().Ingresses(namespace).List(opts)
	if err != nil {
		return nil, err
	}
